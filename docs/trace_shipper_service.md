...
5. In the `Trace Shipper Service` folder, edit the `TraceShipperService.exe.config` file. 
   ...
   2. For each folder that needs to ship, edit each attribute in the <add> node to be correct
      ...
      4. **remoteRelativeConfigPath** - the path relative to the workspace fileshare root of the destination workspace where configuration should be pulled from. This setting is ignored if **retrieveConfigurationIntervalInMinutes** is less than or equal to 0. If this string is empty or missing, defaults to `**remoteRelativePath**\Config`, which should be correct for most situations. Contact support@relativity.com prior to changing this value.
      5. **localConfigDestinationPath** - the full path to the local folder to download the remote config directory to. Note, this will create a subdirectory name `Config` in the configured location, which stores the configuration for the Data Source. If empty or missing, this value defaults to whatever is set for **localDirectoryPath** and should only be changed if necessary.
      ...
      9. **logUploadIntervalInMinutes** - the interval between shipper log pushes to the Relativity server, contact support@relativity.com for more information and before altering this value. 
