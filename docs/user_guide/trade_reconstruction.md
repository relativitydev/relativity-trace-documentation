...
There are several components to Trade Reconstruction. To begin, a Trade Reconstruction Configuration must be created. Trade Reconstruction Configruations are defined by administrators during implementation, more configuration information found [here]({{ site.baseurl }}{% link docs/administrator_guide/trade_reconstruction.md %}). Once a Trade Reconstruction Configuration is created, Trades can be created and reconstructed. Below is how to create a Trade and run Trade Reconstruction.



## Creating A Trade

To run Trade Reconstruction, you need to create a Trade. The following steps are how to create a Trade:

1. Navigate to `Trades` tab and click `New Trade`.

   ![](C:\Users\jaime.becker\Desktop\trace\documentation\relativity-trace-documentation\docs\user_guide\media\trade_reconstruction\trade_layout.PNG)
