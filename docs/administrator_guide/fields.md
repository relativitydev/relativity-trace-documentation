...
21. **Trace Cleansed Extracted Text** - Long Text field which stores the cleansed extracted text content generated during AI Extracted Text Cleansing. If cleansing is turned off, then it will contain the same content as Extracted Text field.

22. **Trace Removed Extracted Text** - Long Test field which stores a JSON of all the email properties that were removed for a given document during AI Extracted Text Cleansing. If cleansing is turned off or if no content was removed during cleansing, then it will always be empty
