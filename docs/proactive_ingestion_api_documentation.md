...
8. `Group Identifier` - cannot be empty. Must have a unique value for every unique native file (ideally a new Guid, should not be a value from a different column). Extracted children of a native file should have the same Group Identifier as the native file. This field is used to group native file and it's children ( for example, an email and it's attachments )
